...
Transformer* init_transformer(int seq_len, int d_model, int hidden_dim, int num_layers, int batch_size, bool is_causal, bool apply_rope, cublasLtHandle_t cublaslt_handle) {
    ...
    for (int i = 0; i < num_layers; i++) {
        transformer->attention_layers[i] = init_attention(seq_len, d_model, batch_size, is_causal, apply_rope, cublaslt_handle);
...
void save_transformer(Transformer* transformer, const char* filename) {
    ...
    // Save attention is_causal and apply_rope flag
    fwrite(&transformer->attention_layers[0]->is_causal, sizeof(bool), 1, file);
    fwrite(&transformer->attention_layers[0]->apply_rope, sizeof(bool), 1, file);
...
Transformer* load_transformer(const char* filename, int custom_batch_size, cublasLtHandle_t cublaslt_handle) {
    ...
    Transformer* transformer = init_transformer(seq_len, d_model, hidden_dim, num_layers, batch_size, is_causal, apply_rope, cublaslt_handle);
