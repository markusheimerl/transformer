...
void backward_pass_transformer(Transformer* transformer, float* d_X, float* d_grad_X) {
    ...
    // Step 1: Backward through second MLP
    backward_pass_mlp(transformer->mlp2, transformer->attention2->d_output, transformer->attention2->d_grad_output);
    
    // Step 2: Add gradient from fourth residual connection (mlp2_output += attention2_output)
    residual_add_kernel<<<num_blocks, block_size>>>(
    ...
    // Step 3: Backward through second attention (gradient flows to mlp1->d_grad_output)
    backward_pass_attention(transformer->attention2, transformer->mlp1->d_layer_output, transformer->mlp1->d_grad_output);
    
    // Step 4: Add gradient from third residual connection (attention2_output += mlp1_output)
    residual_add_kernel<<<num_blocks, block_size>>>(
    ...
    // Step 5: Backward through first MLP (gradient already in mlp1->d_grad_output from step 4)
    backward_pass_mlp(transformer->mlp1, transformer->attention1->d_output, transformer->attention1->d_grad_output);
    
    // Step 6: Add gradient from second residual connection (mlp1_output += attention1_output)
    residual_add_kernel<<<num_blocks, block_size>>>(
    ...
    // Step 7: Backward through first attention
    backward_pass_attention(transformer->attention1, d_X, d_grad_X);
    
    // Step 8: Add gradient from first residual connection (attention1_output += input)
    if (d_grad_X != NULL) {
        residual_add_kernel<<<num_blocks, block_size>>>(
            d_grad_X, 
            transformer->attention1->d_grad_output, 
...
void save_transformer(Transformer* transformer, const char* filename) {
    ...
    // Save attention is_causal flag (assuming both layers have same causal setting)
